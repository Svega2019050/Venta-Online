      ...
      if (err) {
        ...
        Product.findOne({ name: params.name }, (err, productCopy) => {
          if (err) {
            ...
            return res
              .status(401)
              .send({ message: 'Nombre de Producto ya existente' });
          } else {
        ...
        if (productDetect._id == idProduct) {
          Product.findById(idProduct, (err, productFind) => {
            if (err) {
              ...
              Category.findOne(
                { _id: idCategory, product: idProduct },
                (err, categoryFind) => {
                  if (err) {
                    return res
                      .status(500)
...
function updateStockProduct(req, res) {
  ...
  if (!idCategory && !idProduct) {
    ...
    Category.findOne(
      { _id: idCategory, productsId: idProduct },
      (err, productFound) => {
        if (err) {
          return res.status(500).send({ message: 'Error general' });
        } else if (productFound) {
            ...
            if (err) {
              ...
              Product.findByIdAndRemove(idProduct, (err, productRemove) => {
                if (err) {
                  ...
                  return res
                    .status(404)
                    .send({ message: 'No se puedo Eliminar Producto' });
                }
              });
            } else {
              return res
                .status(403)
                .send({ message: 'Producto no encontrada' });
            }
          });
        } else {
